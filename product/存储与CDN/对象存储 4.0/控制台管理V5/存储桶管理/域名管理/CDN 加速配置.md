...
| 公有读              | 关闭         | 开启         | 需使用鉴权 URL | 可访问          | 不推荐       |
| 公有读              | 开启         | 关闭         | 可访问        | 可访问          | 不推荐       |
| 公有读              | 开启         | 开启         | 需使用鉴权 URL | 可访问          | 不推荐       |
| 私有读+CDN 服务授权 | 开启         | 开启         | 需使用鉴权 URL | 需使用 COS 鉴权 | 全链路保护   |
| 私有读+CDN 服务授权 | 关闭         | 开启         | 需使用鉴权 URL | 需使用 COS 鉴权 | 不推荐       |
...
### 设置步骤
#### 默认 CDN 加速
1. 单击存储桶详情界面上方的【域名管理】，单击【编辑】，把默认加速域名的当前状态设置为开启。源站类型通常默认为 XML 节点，如果作为源站的存储桶开启了静态网站，并且希望为静态网站加速，则选择为静态网站节点。
...
3. 单击【保存】之后，“默认加速域名”下会出现 CDN 鉴权状态提示，可通过单击页面上【鉴权配置】开始配置鉴权配置。
